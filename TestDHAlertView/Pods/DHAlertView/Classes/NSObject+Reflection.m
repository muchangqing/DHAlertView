...
{
        return [[self class] dh_propertyKeys];
}
+ (NSArray *)dh_propertyKeys {
...
{
        return [[self class] dh_propertiesInfo];
}
/**
...
+ (NSArray *)dh_propertiesInfo
{
        ...
        for (int i = 0; i < propertyCount; i++)
        {
                [propertieArray addObject:({
                        ...
                        NSDictionary *dictionary = [self dh_dictionaryWithProperty:properties[i]];
...
+ (NSArray *)dh_propertiesWithCodeFormat
{
        ...
        NSArray *properties = [[self class] dh_propertiesInfo];
...
-(NSArray*)dh_methodListInfo{
        ...
        for (int i = 0; i < count ; i++)
        {
                ...
                for (int index=0; index<argumentsCount; index++) {
                        ...
                        [arguments addObject:[[self class] dh_decodeType:arg]];
                }
                
                NSString *returnTypeString =[[self class] dh_decodeType:returnType];
                NSString *encodeString = [[self class] dh_decodeType:encoding];
...
-(NSDictionary *)dh_protocolList{
        return [[self class]dh_protocolList];
}
+ (NSDictionary *)dh_protocolList
{
...
+ (NSArray *)dh_instanceVariable
{
        ...
        NSMutableArray *result = [NSMutableArray array];
        for (int i = 0; i < outCount; i++) {
                NSString *type = [[self class] dh_decodeType:ivar_getTypeEncoding(ivars[i])];
...
#pragma mark -- help
+ (NSDictionary *)dh_dictionaryWithProperty:(objc_property_t)property
{
...
+ (NSString *)dh_decodeType:(const char *)cString
{
        ...
        } else
        {
                if ([[result substringToIndex:1] isEqualToString:@"^"]) {
                        result = [NSString stringWithFormat:@"%@ *",
                                  [NSString dh_decodeType:[[result substringFromIndex:1] cStringUsingEncoding:NSUTF8StringEncoding]]];
                }
        }
        return result;
}
